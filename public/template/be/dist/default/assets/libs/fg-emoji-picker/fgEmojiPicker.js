...
const FgEmojiPicker = function (options) {
    ...
    this.init = () => {
        ...
        this.selectors.trigger = this.options.hasOwnProperty("trigger")
            ? this.options.trigger
            : console.error(
                  "You must proved trigger element like this - 'EmojiPicker.init({trigger: \"selector\"})' "
              );
        ...
        this.variable.removeOnSelection =
            this.options.removeOnSelection || false;
    ...
    this.bindEvents = () => {
        this.lib("body").on(
            "click",
            this.functions.removeEmojiPicker.bind(this)
        );
        this.lib("body").on("click", this.functions.emitEmoji.bind(this));
        this.lib("body").on(
            "click",
            this.functions.openEmojiSelector.bind(this),
            this.selectors.trigger
        );
        this.lib("body").on(
            "input",
            this.functions.search.bind(this),
        ...
        window.addEventListener("keydown", (e) => {
            if (e.keyCode === 27) {
                if (document.querySelector(this.selectors.emojiPicker)) {
                    document
                        .querySelectorAll(this.selectors.emojiPicker)
    ...
    (this.functions = {
        ...
        search(e) {
            ...
            if (!Array.isArray(this.emojiItems)) {
                this.emojiItems = Array.from(
                    e.target
                        .closest(this.selectors.emojiPicker)
                        .querySelectorAll(`${this.selectors.emojiBody} li`)
                );
            }
        ...
        fetchEmojiData: () => {
            fetch(
                `template/be/dist/default/assets/js/pages/plugins/jsonfull-emoji-list.json`
            )
        ...
        removeEmojiPicker() {
            ...
            if (!el.closest(this.selectors.emojiPicker))
                picker ? picker.remove() : false;
            this.emojiItems = undefined;
        },
        ...
        emitEmoji(e) {
            ...
            if (
                ...
                el.className.includes("fg-emoji-picker-item")
            ) {
                e.preventDefault();
                ...
                if (this.variable.emit)
                    this.variable.emit(emoji, this.triggerer);

                // If insert into option exists
                if (this.insertInto)
                    this.functions.putInPlace(this.insertInto, emoji.emoji);
        ...
        openEmojiSelector(e) {
            ...
            if (el) {
                ...
                if (typeof this.variable.emit === "function")
                    this.triggerer = el;

                // Insert picker
                const emojiPickerMain = new DOMParser().parseFromString(
                    this.html.pickerBody.apply(this),
                    "text/html"
                ...
                let position = {
                    top: (emojiPickerMain.style.top =
                        positions.buttonTop - positions.emojiHeight),
                    left: (emojiPickerMain.style.left =
                        positions.buttonFromLeft - positions.emojiWidth),
                    bottom: (emojiPickerMain.style.top = positions.buttonTop),
                    right: (emojiPickerMain.style.left =
                        positions.buttonFromLeft + positions.buttonWidth),
                };
                ...
                if (this.variable.position) {
                    this.variable.position.forEach((elemPos) => {
                        if (elemPos === "right") {
                            emojiPickerMain.style.left =
                                position[elemPos] + "px";
                        } else if (elemPos === "bottom") {
                            emojiPickerMain.style.top =
                                position[elemPos] + "px";
                        } else {
                            emojiPickerMain.style[elemPos] =
                                position[elemPos] + "px";
                        }
                    });
                }
                ...
                this.emojiPicker().then((emojiPicker) => {
                    ...
                    const emojiFooter = emojiPickerMain.querySelector(
                        this.selectors.emojiFooter
                    );
                    const emojiBody = emojiPickerMain.querySelector(
                        this.selectors.emojiBody
                    );
                    const emojiPickerHeader = emojiPickerMain.querySelector(
                        this.selectors.emojiHeader
                    );
                    ...
                    document.body.querySelector(
                        ...
                        document
                            .querySelectorAll(".fg-emoji-picker-categories li")
                            .forEach((item) => item.classList.remove("active"));

                        if (
                            filterLlnk &&
                            filterLlnk.closest("li") &&
                            filterLlnk.closest("li").getAttribute("data-index")
                        ) {
                            ...
                            scrollTo(
                                emojiBody,
                                emojiBody.querySelector(`#${listIndex}`)
                                    .offsetTop - emojiPickerHeader.offsetHeight
                            );
                        }
                    };
                });
        ...
        <div class="fg-emoji-picker">
            ...
            const closeBtn = this.variable.closeButton
                ? `<li><a class="fg-emoji-picker-close-button" href="#">${this.variable.categoryIcons.close}</a></li>`
                : "";
            ...
            if (this.emojiJson) {
                fetchData = new Promise((resolve, reject) => {
                    ...
                    for (const key in object) {
                        if (object.hasOwnProperty(key)) {
                            ...
                            categoriesInner += `<li class="${
                                index === 1 ? "active" : ""
                            }" id="${keyToId}" data-index="${keyToId}"><a href="#${keyToId}">${
                                this.variable.categoryIcons[keyToId]
                            }</a></li>`;

                            innerLists += `
                            <ul class="fg-emoji-picker-category ${
                                index === 1 ? "active" : ""
                            }" id="${keyToId}" category-name="${key}">
                                <div class="fg-emoji-picker-container-title">${key}</div>
                                <div class="fg-emoji-picker-grid">`;

                            // Loop through emoji items
                            categories.forEach((item) => {
                                innerLists += `<li data-name="${item.description.toLowerCase()}"><a class="fg-emoji-picker-item" title="${
                                    item.description
                                }" data-name="${item.description.toLowerCase()}" data-code="${
                                    item.code
                                }" href="${item.emoji}">${item.emoji}</a></li>`;
                            });
                            innerLists += `
                                </div>
                            </ul>`;
                    ...
                    let cats = categories.replace(
                        "%categories%",
                        categoriesInner
                    );
                    let pickerContainer = picker.replace(
                        "%pickerContainer%",
                        allSmiles
                    );
                ...
                fetchData = fetch(`${this.variable.dir}full-emoji-list.json`)
                    ...
                    .then((object) => {
                        ...
                        let data = pickerContainer.replace(
                            "%categories%",
                            cats
                        );
                        return data;
