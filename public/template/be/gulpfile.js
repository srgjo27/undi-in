...
gulp.task('watch', async function () {
    gulp.watch([paths.src.scss.files, '!' + paths.src.custom.files, '!' + paths.src.icon.files,  '!' + paths.src.bootstrap.typeFiles, '!' + paths.src.bootstrap.light, '!' + paths.src.bootstrap.variables], gulp.series('scss', 'browsersyncReload'));
    gulp.watch([paths.src.bootstrap.typeFiles, '!' + paths.src.bootstrap.components, '!' + paths.src.bootstrap.light], gulp.series('bootstrap', 'browsersyncReload'));
    gulp.watch([paths.src.bootstrap.light, paths.src.bootstrap.variables], gulp.series('bootstrap', 'scss', 'browsersyncReload'));
    ...
    if (envConfig.run != "all") {
        ...
        return new Promise(function (resolve, reject) {
            for (let i = 0; i < envConfig.demos.length; i++) {
                var destPath = paths.dist.js.dir;
                destPath = destPath.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.demos[i]);
...
gulp.task('jsPages', function () {
    if (envConfig.run != "all") {
        return new Promise(function (resolve, reject) {
            ...
            JsPagePath = JsPagePath.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.run);
        ...
        return new Promise(function (resolve, reject) {
            ...
            for (let i = 0; i < envConfig.demos.length; i++) {
                var JsPagePath = paths.dist.js.files;
                JsPagePath = JsPagePath.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.demos[i]);
                var proc = gulp.src(paths.src.js.files).pipe(uglify()).pipe(gulp.dest(JsPagePath));
                if (i == envConfig.demos.length - 1)
                    resolve(proc);
            }
...
gulp.task('icon', async function () {
    if (envConfig.run != "all") {
        ...
        for (let i = 0; i < envConfig.demos.length; i++) {
            var iconFiles = paths.src.icon.dir;
            iconFiles = iconFiles.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.demos[i]);
